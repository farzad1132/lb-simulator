...
| **`--no-bind` (oldest-FCFS approx pulls)** | yes | yes | Outbound approx only in `ms`; see [approx-policy.md § No-bind mode](approx-policy.md#no-bind-mode---no-bind) |
| **`--approx-sched edf` (outbound approx queue)** | — | yes | `ms` only; requires `approx` + `--no-bind`; see [approx-policy.md](approx-policy.md) |
