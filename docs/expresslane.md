...
**`--lb-policy centralized` and `--lb-policy amphiqueue` are not supported with `--expresslane`.** The simulator rejects both combinations at startup. Express lane requires push-based client load balancers and regular-server queue eviction; centralized and amphiqueue policies hold backlog at client load balancers with no regular-server task queues.
