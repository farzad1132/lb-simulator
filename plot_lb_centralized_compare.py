...
def select_configs(args: argparse.Namespace) -> list[ExperimentConfig]:
    ...
    for idx in args.config_index:
        if idx < 0 or idx >= len(DEFAULT_CONFIGS):
            raise SystemExit(
                f"--config-index {idx} out of range "
                f"(0 .. {len(DEFAULT_CONFIGS) - 1})"
            )
        configs.append(DEFAULT_CONFIGS[idx])
    return configs
