...
fn fanin_f1_request_flow_trace() {
    ...
    assert_in_order(
        ...
        &[
            ...
            "EdgeBalancer(api=f1)",
            "Replica(frontend/0) serve start endpoint=frontend:f1",
            "ReplicaBalancer(frontend/0) outbound target=backend1",
            "Replica(backend1/0) serve start endpoint=backend1:f2",
            "ReplicaBalancer(backend1/0) outbound target=shared",
            "Replica(shared/0) serve start endpoint=shared:f5",
            "return -> frontend/0 resume=frontend:f1 sibling=1",
            "ReplicaBalancer(frontend/0) outbound target=backend2",
            "Replica(backend2/0) serve start endpoint=backend2:f4",
